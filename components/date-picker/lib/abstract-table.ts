...
import { isSameDate, isSameRange } from 'ng-zorro-antd/date-picker';
...
export abstract class AbstractTable implements OnInit, OnChanges {
  ...
  @Input() selectedValue: (Date | null)[] = []; // Range ONLY
  @Input() hoverValue: (Date | null)[] = []; // Range ONLY
  @Input() innerDisabledDate?: (d: Date) => boolean; // Range ONLY
  ...
  private isDateRealChange(change: SimpleChange): boolean {
    if (change) {
      ...
      if (Array.isArray(currentValue)) {
        return !isSameRange(previousValue as Date[], currentValue);
      } else {
        return !isSameDate(previousValue as Date, currentValue);
      }
    }
    return false;
  }
